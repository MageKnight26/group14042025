...
class Student(Human):
    ...
    def __hash__(self):  # дозволяє використовувати Student у set()
        return hash((self.first_name, self.last_name, self.record_book))

    def __eq__(self, other):  # потрібен для унікальності в set
        return (
...
assert isinstance(gr.find_student("Jobs"), Student) is True, "Повернення екземпляра"
